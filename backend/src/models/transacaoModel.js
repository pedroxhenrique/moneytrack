...
const listar = async (usuarioId, filtros = {}) => {
  let sql = `
    SELECT t.id, t.tipo, t.valor, t.descricao, t.data,
           t.categoria_id, t.criado_em, c.nome as categoria_nome
    FROM transacoes t
...
const criar = async (dados) =>
  db.insert(
    `INSERT INTO transacoes (tipo, valor, descricao, data, categoria_id, usuario_id)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [dados.tipo, dados.valor, dados.descricao || '', dados.data, dados.categoriaId, dados.usuarioId]
  );

const atualizar = async (id, dados, usuarioId) =>
  db.run(
    `UPDATE transacoes SET tipo=?, valor=?, descricao=?, data=?, categoria_id=?
     WHERE id=? AND usuario_id=?`,
    [dados.tipo, dados.valor, dados.descricao || '', dados.data, dados.categoriaId, id, usuarioId]
  );
